...
- **IMAP via `imapflow`**, patched for a workerd stream-timing quirk (`patches/imapflow@1.7.8.patch` — keep it in sync when bumping). SMTP is implemented over sockets in `server/email/smtp.ts`.
