...
- **IMAP via `imapflow`**, patched for a workerd stream-timing quirk (`patches/imapflow+1.7.8.patch`, applied by `patch-package` via the `postinstall` hook — run by both `npm install` and `bun install`; regenerate with `npx patch-package imapflow` when bumping). SMTP is implemented over sockets in `server/email/smtp.ts`.
  - Don't pass `tls.rejectUnauthorized` to imapflow (workerd throws `ERR_OPTION_NOT_IMPLEMENTED`); pass an explicit `servername`.
  - The imapflow patch (in `patches/imapflow+1.7.8.patch`) also fixes a workerd `ImapStream` input-drain race that made large responses hang.
