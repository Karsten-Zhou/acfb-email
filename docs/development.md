...
```bash
bunx wrangler d1 execute acfb-email --local --file=./migrations/0001_initial.sql
```

> During early development, editing an already-applied migration requires a local DB reset: delete `.wrangler/state` and re-apply (a reset also clears locally stored email accounts).
